...
public class StatisticsController : ControllerBase
{
	...
	{
		if(!this.ParseAdminClaim()) return NotFound();

		return await Task.Run(() => Ok(service.EnpointToRequestsTime));
	}
	...
	public async Task<IActionResult> GetCounts([FromServices] StatisticsService service)
	{
		...
		return await Task.Run(() => Ok(service.EnpointToRequestsTime.ToDictionary(x => x.Key, x => x.Value.Count)));
	}
}
